...
// Using standard CSS styles for Popover/Tooltip
...
const TooltipContent = React.forwardRef<React.ElementRef<typeof TooltipPrimitive.Content>, React.ComponentPropsWithoutRef<typeof TooltipPrimitive.Content> & { showArrow?: boolean }>(({ className, sideOffset = 4, showArrow = false, ...props }, ref) => (
  <TooltipPrimitive.Portal>
    <TooltipPrimitive.Content ref={ref} sideOffset={sideOffset} style={baseFloatingStyle} {...props}>
      {props.children}
      {showArrow && <TooltipPrimitive.Arrow style={{ fill: "var(--line)" }} />}
...
const PopoverContent = React.forwardRef<React.ElementRef<typeof PopoverPrimitive.Content>, React.ComponentPropsWithoutRef<typeof PopoverPrimitive.Content>>(({ className, align = "center", sideOffset = 4, ...props }, ref) => (
  <PopoverPrimitive.Portal>
    <PopoverPrimitive.Content ref={ref} align={align} sideOffset={sideOffset} style={{ ...baseFloatingStyle, padding: "8px", width: "220px", borderRadius: "12px", outline: "none" }} {...props} />
  </PopoverPrimitive.Portal>
));
...
const DialogOverlay = React.forwardRef<React.ElementRef<typeof DialogPrimitive.Overlay>, React.ComponentPropsWithoutRef<typeof DialogPrimitive.Overlay>>(({ className, ...props }, ref) => (
  <DialogPrimitive.Overlay ref={ref} style={{ position: "fixed", inset: 0, zIndex: 50, backgroundColor: "rgba(0,0,0,0.6)", backdropFilter: "blur(4px)" }} {...props} />
));
...
const DialogContent = React.forwardRef<React.ElementRef<typeof DialogPrimitive.Content>, React.ComponentPropsWithoutRef<typeof DialogPrimitive.Content>>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    <DialogOverlay />
    <DialogPrimitive.Content ref={ref} style={{ position: "fixed", left: "50%", top: "50%", zIndex: 50, transform: "translate(-50%, -50%)", width: "100%", maxWidth: "90vw" }} {...props}>
      <div style={{ backgroundColor: "var(--bg)", borderRadius: "28px", overflow: "hidden", padding: "4px", position: "relative" }}>
        {children}
        <DialogPrimitive.Close style={{ position: "absolute", right: "12px", top: "12px", zIndex: 10, borderRadius: "50%", background: "var(--panel)", border: "none", color: "var(--text)", padding: "4px", cursor: "pointer" }}>
...
export const PromptBox = React.forwardRef<HTMLTextAreaElement, React.TextareaHTMLAttributes<HTMLTextAreaElement> & { onSend?: () => void }>(
  ({ className, onSend, onChange, value, ...props }, ref) => {
    ...
    // Map Tailwind structure to Vanilla CSS
    return (
      <div
        className={className}
        style={{
      ...
      >
        ...
        {imagePreview && (
          <Dialog open={isImageDialogOpen} onOpenChange={setIsImageDialogOpen}>
            <div style={{ position: "relative", marginBottom: "4px", width: "fit-content", borderRadius: "16px", padding: "4px" }}>
              <button type="button" onClick={() => setIsImageDialogOpen(true)} style={{ background: "none", border: "none", padding: 0, cursor: "pointer" }}>
                <img src={imagePreview} alt="Image preview" style={{ height: "58px", width: "58px", borderRadius: "16px", objectFit: "cover" }} />
              </button>
              <button onClick={handleRemoveImage} style={{ position: "absolute", right: "8px", top: "8px", zIndex: 10, display: "flex", alignItems: "center", justifyContent: "center", borderRadius: "50%", background: "var(--panel-solid)", border: "1px solid var(--line)", color: "var(--text)", padding: "4px", cursor: "pointer" }}>
                <XIcon width={16} height={16} />
              </button>
            </div>
            <DialogContent>
              <img src={imagePreview} alt="Full size preview" style={{ width: "100%", maxHeight: "95vh", objectFit: "contain", borderRadius: "24px" }} />
            </DialogContent>
          </Dialog>
        )}
        ...
        <div style={{ marginTop: "2px", padding: "4px", paddingTop: 0 }}>
          <TooltipProvider delayDuration={100}>
            <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
              
              <Tooltip>
                <TooltipTrigger asChild>
                  <button type="button" onClick={handlePlusClick} style={{ display: "flex", height: "32px", width: "32px", alignItems: "center", justifyContent: "center", borderRadius: "50%", color: "var(--text)", background: "transparent", border: "none", cursor: "pointer" }}>
              ...
              <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen}>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <PopoverTrigger asChild>
                      <button type="button" style={{ display: "flex", height: "32px", alignItems: "center", gap: "8px", borderRadius: "99px", padding: "8px", fontSize: "14px", color: "var(--text)", background: "transparent", border: "none", cursor: "pointer" }}>
                ...
                <PopoverContent side="top" align="start">
                  <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
                    {toolsList.map(tool => (
                      <button key={tool.id} onClick={() => { setSelectedTool(tool.id); setIsPopoverOpen(false); }} style={{ display: "flex", width: "100%", alignItems: "center", gap: "8px", borderRadius: "6px", padding: "8px", textAlign: "left", fontSize: "14px", color: "var(--text)", background: "transparent", border: "none", cursor: "pointer" }}>
                        <tool.icon width={16} height={16} />
                        <span>{tool.name}</span>
                        {tool.extra && <span style={{ marginLeft: "auto", fontSize: "12px", color: "var(--text-muted)", opacity: 0.7 }}>{tool.extra}</span>}
                      </button>
                    ))}
                  </div>
                </PopoverContent>
              ...
              {activeTool && (
                <>
                  <div style={{ height: "16px", width: "1px", backgroundColor: "var(--line)" }} />
                  <button onClick={() => setSelectedTool(null)} style={{ display: "flex", height: "32px", alignItems: "center", gap: "8px", borderRadius: "99px", padding: "0 8px", fontSize: "14px", color: "var(--accent)", background: "var(--accent-muted)", border: "none", cursor: "pointer" }}>
                    {ActiveToolIcon && <ActiveToolIcon width={16} height={16} />}
                  ...
                  <TooltipTrigger asChild>
                    <button
                      ...
                      style={{
                        ...
                        transition: "all 0.2s",
                        border: "none",
                        backgroundColor: hasValue ? "var(--text)" : "var(--panel)",
                        color: hasValue ? "var(--bg)" : "var(--line)",
